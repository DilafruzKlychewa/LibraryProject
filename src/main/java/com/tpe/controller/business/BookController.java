...
public class BookController {
    ...
    //1.ENDPOINT GET
    @GetMapping("/books")  //http://localhost/8081/books?q=sefiller&cat=4&author=34&publisher=42&page=1&size=10&sort=name&type=asc
    ...
    //2. ENDPOINT GET
    @GetMapping("/{bookId}") //http://localhost/8081/books
    ...
    @PreAuthorize("hasAnyAuthority('ADMIN')")
    @PutMapping("/books/{id}") //http://localhost:8081//books/5
    ...
    @PreAuthorize("hasAnyAuthority('ADMIN')")
    @DeleteMapping("/books/{id}") // http://localhost:8081/meet/2
